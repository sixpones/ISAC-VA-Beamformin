...
def main():
    ...
    axs[0].axvline(x=26.7, color='r', linestyle='--', label='LoS/NLoS Boundary (Y=26.7)')
    axs[0].legend()
    axs[0].grid(True)

    axs[1].plot(y_coords, max_tau, 'go-', markersize=4)
    axs[1].set_ylabel('ToA (seconds)')
    axs[1].axvline(x=26.7, color='r', linestyle='--')
    axs[1].grid(True)

    axs[2].plot(y_coords, max_phi_deg, 'ro-', markersize=4)
    axs[2].set_ylabel('AoA Azimuth (degrees)')
    axs[2].set_xlabel('UE Y-Coordinate (m)')
    axs[2].axvline(x=26.7, color='r', linestyle='--')
    axs[2].grid(True)

    plt.tight_layout()
    plt.savefig("multipath_sanity_check.png", dpi=300)
    print("Plot saved to multipath_sanity_check.png")
